...
class MatchedViewController: UIViewController {
    ...
    func unmatchAlert() {
        ...
        alert.addButton("I am sure") {
            ...
            self.thisNeedsToFinishBeforeWeCanDoTheNextStep { () -> () in
    ...
    func thisNeedsToFinishBeforeWeCanDoTheNextStep(completion: () -> ()) {
        ...
        DispatchQueue.main.async {
            ...
            currentUserRef.observeSingleEvent(of: .value) { (snapshot) in
                ...
                ref.child("Loved").child((user?.phoneNumber)!).child("Followers").child(oldCrush).removeValue()
                
                // delete old crush's status
                ref.child("Users").child(oldCrush).child("Status").removeValue()
                
                // delete user from oldCrush's crush
                ref.child("Users").child(oldCrush).child("CrushNumber").removeValue()
            }
        }
        completion()
    }
